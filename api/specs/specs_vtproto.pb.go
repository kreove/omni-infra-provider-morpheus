...
func (this *MachineSpec) EqualVT(that *MachineSpec) bool {
	...
	if this.ConfigToken != that.ConfigToken {
		return false
	}
...
func (m *MachineSpec) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.ConfigToken) > 0 {
		i -= len(m.ConfigToken)
		copy(dAtA[i:], m.ConfigToken)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.ConfigToken)))
		i--
		dAtA[i] = 0x32
	}
...
func (m *MachineSpec) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 6:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field ConfigToken", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.ConfigToken = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		default:
			iNdEx = preIndex
